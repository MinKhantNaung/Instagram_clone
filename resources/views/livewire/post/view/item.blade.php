        ...
        <footer class="mt-auto sticky border-t bottom-0 z-10 bg-white">
            ...
            <div class="flex gap-4 items-center my-2">
                ...
                @if ($post->isLikedBy(auth()->user()))
                    <button wire:click.prevent='togglePostLike()'>
                ...
                <span class="ml-auto">
                    ...
                    @if ($post->hasBeenFavoritedBy(auth()->user()))
                        <button wire:click.prevent='togglePostFavorite()'>
            ...
            @if ($post->totalLikers > 0 && !$post->hide_like_view)
                <p class="font-bold text-sm">{{ $post->totalLikers }} {{ $post->totalLikers > 0 ? 'likes' : 'like' }}</p>
            @endif
            ...
            @if ($post->allow_commenting)
                ...
                <form wire:key='{{ time() }}' x-data="{ body: @entangle('body') }" @submit.prevent="$wire.addComment()"
                    ...
                    <div class="col-span-1 ml-auto flex justify-end text-right">
                        <button x-cloak wire:loading.attr='disabled' x-show="body.length > 0"
