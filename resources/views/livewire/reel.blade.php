<div class="lg:px-8 flex flex-col h-full my-auto overflow-hidden">
    ...
    <ul class="h-[calc(100vh-1rem)] my-auto flex flex-col gap-y-7 snap-y snap-mandatory overflow-x-hidden">
        ...
        @foreach ($posts as $post)
            ...
            <li
                class="h-[calc(100vh-5rem)] max-w-sm mx-auto w-full cursor cursor-pointer rounded-lg
             relative group shrink-0 snap-center snap-always grid grid-cols-12 gap-4">

                <div class="col-span-10 sm:col-span-11 bg-black border">
                    <x-video :controls="true" :cover="false" source="{{ $cover->url }}" />
                </div>

                {{-- actions --}}
                <div class="col-span-2 sm:col-span-1 flex flex-col gap-y-8 gap-x-0 items-center justify-end">
                    ...
                    <span class="-ml-px">
                        ...
                        @if ($post->hasBeenFavoritedBy(auth()->user()))
                            <button wire:click="togglePostFavorite({{ $post->id }})">
                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor"
                                    class="w-6 h-6">
